...
func (e Gen) Preview(c *gin.Context) {
	...
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Failed to receive tableId! Error details"), err.Error()))
		return
	}
	table.TableId = id
	var TemplateVer = global.TemplateVer //template ver
	t1, err := template.ParseFiles("template/" + TemplateVer + "/model.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Model template reading failed! Error details"), err.Error()))
		return
	}
	t2, err := template.ParseFiles("template/" + TemplateVer + "/no_actions/apis.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "API template reading failed! Error details"), err.Error()))
		return
	}
	t3, err := template.ParseFiles("template/" + TemplateVer + "/js.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Js template reading failed! Error details"), err.Error()))
		return
	}
	t4, err := template.ParseFiles("template/" + TemplateVer + "/vue.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Vue template reading failed! Error details"), err.Error()))
		return
	}
	t5, err := template.ParseFiles("template/" + TemplateVer + "/no_actions/router_check_role.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Failed to read routing template! Error details"), err.Error()))
		return
	}
	t6, err := template.ParseFiles("template/" + TemplateVer + "/dto.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, err.Error())
		return
	}
	t7, err := template.ParseFiles("template/" + TemplateVer + "/no_actions/service.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, err.Error())
		return
	...
	if err != nil {
		...
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Failed to obtain database link! Error details"), err.Error()))
		return
	}
	...
	mp["template/init_router.template"] = b8.String()
	mp["template/router.template"] = b9.String()
	mp["template/api_router.template"] = b10.String()
...
func (e Gen) NOActionsGen(c *gin.Context, tab tools.SysTables) {
	...
	basePath := "template/" + global.TemplateVer + "/"
	...
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Views directory creation failed! Error details"), err.Error()))
		return
	}
	...
	pkg.FileCreate(b8, "./app/"+tab.PackageName+"/router/init_router.go") //auto create
	pkg.FileCreate(b9, "./app/"+tab.PackageName+"/router/router.go")      //auto create
	pkg.FileCreate(b10, "./cmd/admin/"+tab.MLTBName+".go")                // add router into default app
...
func (e Gen) genApiToFile(c *gin.Context, tab tools.SysTables) {
	...
	if err != nil {
		e.Logger.Error(err)
		e.Error(500, err, fmt.Sprintf(ginI18n.MustGetMessage(c, "Data migration template parsing failed! Error details"), err.Error()))
		return
	}
...
func (e Gen) GenMenuAndApi(c *gin.Context) {
	...
	MList.MenuName = ""
	MList.Title = "Query " + tab.TableComment
	MList.Icon = ""
	...
	MCreate.MenuName = ""
	MCreate.Title = "Add " + tab.TableComment
	MCreate.Icon = ""
	...
	MUpdate.MenuName = ""
	MUpdate.Title = "Edit " + tab.TableComment
	MUpdate.Icon = ""
	...
	MDelete.MenuName = ""
	MDelete.Title = "Remove " + tab.TableComment
	MDelete.Icon = ""
	...
	e.OK("", ginI18n.MustGetMessage(c, "Data generation successful"))
}
